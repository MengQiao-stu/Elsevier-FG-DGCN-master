...
pair_wise_distance_cuda = load_inline("pair_wise_distance", cpp_sources="", cuda_sources=source)
...
class PairwiseDistFunction(torch.autograd.Function):
    ...
    def forward(self, pixel_features, spixel_features, init_spixel_indices, num_spixels_width, num_spixels_height):
        ...
        output = pixel_features.new(pixel_features.shape[0], 9, pixel_features.shape[-1]).zero_() # 像素与超像素之间的软关联矩阵计算非常昂贵，因此只计算周围9个
        # print(output.shape) [6,9,40000]
